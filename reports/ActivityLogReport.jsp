...
//			document.getElementById("todate").value="";
	...
	document.forms[0].action="/reports/StatusLogAction.do?action=activityreport";
...
<table align="left" class="wrapperDataTable" cellpadding="0" cellspacing="0">
	<tr>
		<td  class="wrapperDataTableTR" align="center" > Reports  >>  User Activity Log Report </td>
	</tr>
	<tr>
		<td align="center">
		...
		<tr class="miniStmtBack">
			 ...
			 <table align="center" border ="0" width="100%">
				<tr>
					...
					<td class="normalText">
						Specify User activity
					 </td>
					 <td>:&nbsp;<html:select property="selectval" >
					<%if(actvtlist!=null){
						for(int i=0;i<actvtlist.length;i++){%>
						 <html:option   value="<%=actvtlist[i]%>"> <%=actvtlist[i]%></html:option>
					<%}}%>
					</html:select>
					</td>
			 	</tr>
			   ...
			   <%
				...
				renderCalendar(IBfromdate);
				   ...
				   <!--  <div	id="cal_todate_display_year" style="display:none;position:absolute;top:250px;left:508px;"> 
 	    			<input	type="text" class="fancy" id="todate_year"  value="" maxLength="4" size="5" tabIndex ='4' style="background-color:#CCCCEE;"/>
			   </div>	
			   <div id="cal_todate_display" style="display:none;position:absolute;top:262px;left:550px;"> 
				 ...
				 <tr>
					<td class="normalText">
					 ...
					 <td>:&nbsp;<html:select property="status" >
						 ...
						 <html:option value="success">Successful </html:option>
						 <html:option value="unsuccess">Unsuccessful</html:option>
					</html:select>
					</td> 
			...
			<table border="2" bordercolor="#ffffff" align="center" width="90%" cellpadding="0" cellspacing="0" id="table">
				<TR>
						  ...
						  Activity Date&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Time</TD>
  					      <TD  nowrap class="dataTableTH">Activity  </TD>
			...
			<%}%>	
				<tbody>
					<tr  <%out.println(i%2==0?"class=trforReportRow1":"class=trforReportRow2");%>>
						...
						<td nowrap class="dataTableTD"><%out.print(DateTimeFunction.ibStrDatetoStr(userlist[i][4]));%></td>
						<%if(userlist[i][5].equals("3")){%>
						<td nowrap class="dataTableTD">Mini Statment</td>
						<%}%>
						<%if(userlist[i][5].equals("4")){%>
						<td nowrap class="dataTableTD">Detail Statement
						
						</td>
						<%}%>
						<%if(userlist[i][5].equals("5")){%>
						<td nowrap class="dataTableTD">Fund Transfer</td>
						<%}%>
						<%if(userlist[i][6].equals("Y")){%>
						<td nowrap class="dataTableTD">Successful</td>
						<%}else{%>
						<td nowrap class="dataTableTD">unsuccessful</td>
						<%}%>
