/**
Shailendra Mishra:
	This is java script is for create and manage tree.
	Description is available with functions.
*/
...
{ 
  ...
  { 
	if (USEFRAMES)
	  docW = docW + "<a href='javascript:clickOnNode(\""+this.getID()+"\")' "
	else
	  docW = docW + "<a href='javascript:clickOnNode(\""+this.getID()+"\")' "
...
function finalizeCreationOfItem(itemArray)
{
  ...
  itemArray.iconSrc = ICONPATH + "images/ftv2doc.gif" 
...
{ 
  ...
  //this is added for checkbox buttons --- shailendra
  var chkboxmnu = this.link.split("~"); 
 // alert(chkboxmnu[0]+":::"+chkboxmnu[1]);
  docW= docW + " <input type='checkbox' name='"+this.link+'~'+this.desc+"' onclick='changeTextLine(this)' style='border:none;' id = 'chkbox'+chkboxmnu[1] "

  //here i m making intial stage of checkbox: will be checked if already in favorite menu, else not.
  var isChecked=false;
  var favMnuNext=this.link.split("~")[0];
  var favListRef=parent.favList;
  var favlen=favListRef.length;
   for(var i=0;i<favlen;i++)
   {
	  if (favMnuNext==favListRef[i])
	  {
		isChecked=true;
		break;
	  }
...
{ 
  ...
  /* to highlight folder -- shailendra
...
function preLoadIcons() {
       arImageSrc = new Array (
           "images/ftv2vertline.gif",
           "images/ftv2mlastnode.gif",
           "images/ftv2mnode.gif",
           "images/ftv2plastnode.gif",
           "images/ftv2pnode.gif",
           "images/ftv2blank.gif",
           "images/ftv2lastnode.gif",
           "images/ftv2node.gif",
           "images/ftv2folderclosed.gif",
           "images/ftv2folderopen.gif",
           "images/ftv2doc.gif"
           )
...
// browser.js, imported in the main html page (left frame).
...
// by shailendra for input line view;
...
function changeTextLine(Lineobj)   
{ 	
	debugger; 
	var docId = Lineobj.id;
	var id1 = ''; 
	var str = Lineobj.name;
	var nodeData=str.split("~"); 
	var aa =nodeData[0];
    id1 = document.getElementById("aA").value;
	var val =  nodeData[0] + '-' + nodeData[1];
	if(id1.indexOf(val)!=-1){
		//var i="@"+id1.indexOf(id);
		//var len=id1.length+1;
		id1=id1.replace("@"+val,"");
	}if(FinalMnuData.indexOf(val)!=-1){
		//var i="@"+id1.indexOf(id);
		//var len=id1.length+1;
		FinalMnuData=FinalMnuData.replace("@"+val,"");
	} else {

	if(id1.trim() == ''){
		id1 =  nodeData[0] + '-' + nodeData[1];
		FinalMnuData+='@'+id1;   
		} else
			{
		id1 = nodeData[0] + '-' + nodeData[1];
	...
	document.getElementById("aA").value = id1;
	//if(Lineobj.checked)// for insert
	//	parent.frame2.addtext(nodeData[0],nodeData[1],nodeData[2])// parameters: mnu_next, mnu_sno, mnu_hname/mnu_ename
	// else // for remove
		// parent.frame2.removetext(nodeData[0]);
		// parameters: mnu_next
}  
...
function getuserid()
{
var e = document.getElementById("cbUsrcsSName");
suserId = e.options[e.selectedIndex].value;
alert("struser-->"+suserId);
...
function sendData()  
{   
 alert("senddata::"+FinalMnuData);
 var ran = '0000';
 var url ; 
 url = "/menu/AddFavoriteClassProcess.do?action=execute&ajaxData=y&FinalMnuData="+FinalMnuData+"&suserId="+suserId+"&ran="+ran;
 var murl=url ; 
 console.log = murl;
	 http.open("POST",murl,false);
...
function doCancle()
{
	window.location.href = "/jsp/login.do?action=ibUsrLogin";
}