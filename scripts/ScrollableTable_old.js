...
function setRowWidth(r){
	//alert(" in setRowWidth"+r.length)
    for (var i=0; i < r.length; i++){
	//	alert("r[i].width "+r[i].width)
	//	alert("r[i].actualWidth "+r[i].actualWidth)
        r[i].width = r[i].actualWidth;
		//r[i].width = 59;


        r[i].innerHTML = '<span style="width:' + r[i].actualWidth + ';">' + r[i].innerHTML + '</span>';
	//	alert("r[i].innerHTML "+r[i].innerHTML);
    }
}

function fixTableWidth(tbl){
 //   alert("inside Function Table is --"+tbl);
	for (var i=0; i < tbl.tHead.rows.length; i++)  
		findRowWidth(tbl.tHead.rows[i].cells);

 //   alert("After Loop");
    findRowWidth(tbl.tBodies[0].rows[0].cells);
//	alert("After Row Width -- Footer is ---"+tbl.tFoot);
    if (tbl.tFoot)  
		for (var i=0; i < tbl.tFoot.rows.length; i++) 
		findRowWidth(tbl.tFoot.rows[i].cells);

//	alert("After Table Foot check ");	
    //tbl.width = '';
//	alert("row length "+tbl.tHead.rows.length);
    for (var i=0; i < tbl.tHead.rows.length; i++)  
		setRowWidth(tbl.tHead.rows[i].cells);

 //    alert(" tbl.tBodies[0].rows[0].cells "+tbl.tBodies[0].rows[0].cells);
    setRowWidth(tbl.tBodies[0].rows[0].cells);
//	alert("tbl.tFoot.rows.length "+tbl.tFoot.rows.length)
