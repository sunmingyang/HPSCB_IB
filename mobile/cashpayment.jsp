...
function saveData()
	{
	  ...
	  var agmobile=document.getElementById("mob").value;
	   var ran=parseInt(Math.random()*9999);
	var url="/TransactionAction.do?action=postAgentCashPayment&beneMobile="+mobileno+"&tranKid="+agnkid+"&userID="+userID+"&sessionID=<%=session.getId()%>&agOTP="+agOTP+"&beneOTP="+beneOTP+"&agmobile="+agmobile+"&ran="+ran;
		http.open("POST",url,false);
			...
			if (http.readyState == 4)
			{
				if (http.status == 200)
				{
					   ...
					   if(jsonData=="-1"){
						...
						window.location.href = "/fundtransfer/cashpayment.jsp";
